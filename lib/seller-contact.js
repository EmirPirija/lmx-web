...
const toE164WithCountryCode = (rawPhone, countryCode) => {
  ...
  if (raw.startsWith("00")) {
    const normalizedInternational = digits.replace(/^00+/, "");
    return normalizedInternational ? `+${normalizedInternational}` : "";
  }
...
export const formatSellerPhoneForDisplay = (value) => {
...
export const resolveSellerCountryCode = ({
  seller = {},
  settings = {},
} = {}) => {
  const rawCountryCode = pickDefined(
    settings?.country_code,
    settings?.countryCode,
    settings?.phone_country_code,
    settings?.phoneCountryCode,
    settings?.mobile_country_code,
    settings?.mobileCountryCode,
    settings?.user?.country_code,
    settings?.user?.countryCode,
    seller?.country_code,
    seller?.countryCode,
    seller?.phone_country_code,
    seller?.phoneCountryCode,
    seller?.mobile_country_code,
    seller?.mobileCountryCode,
    seller?.user?.country_code,
    seller?.user?.countryCode,
  );
  const normalizedCountryCode = normalizeCountryCode(rawCountryCode);
  if (normalizedCountryCode) return normalizedCountryCode;

  const normalizedRawCountryText = String(rawCountryCode || "")
    .trim()
    .toLowerCase();
  if (REGION_TO_DIAL_CODE[normalizedRawCountryText]) {
    return REGION_TO_DIAL_CODE[normalizedRawCountryText];
  }

  const rawRegionCode = pickDefined(
    settings?.region_code,
    settings?.regionCode,
    settings?.phone_region_code,
    settings?.phoneRegionCode,
    settings?.mobile_region_code,
    settings?.mobileRegionCode,
    settings?.user?.region_code,
    settings?.user?.regionCode,
    seller?.region_code,
    seller?.regionCode,
    seller?.phone_region_code,
    seller?.phoneRegionCode,
    seller?.mobile_region_code,
    seller?.mobileRegionCode,
    seller?.user?.region_code,
    seller?.user?.regionCode,
  );
  const normalizedRegionCode = String(rawRegionCode || "")
    .trim()
    .toLowerCase();
  if (REGION_TO_DIAL_CODE[normalizedRegionCode]) {
    return REGION_TO_DIAL_CODE[normalizedRegionCode];
  }

  return REGION_TO_DIAL_CODE.ba;
};
...
export const resolveSellerPhone = ({ seller = {}, settings = {} } = {}) => {
  ...
  const countryCode = resolveSellerCountryCode({ seller, settings });
  return toE164WithCountryCode(value, countryCode);
};

export const resolveSellerContactNumber = ({
  value,
  seller = {},
  settings = {},
} = {}) => {
...
export const resolvePhoneVerificationFromSources = (...sources) => {
  ...
  for (const source of flatSources) {
    const result = resolvePhoneVerifiedFlag({
      seller: source,
      settings: source,
    });
...
export const resolveSellerPhoneContactState = ({
  ...
  return {
    state,
    phone,
    formattedPhone: formatSellerPhoneForDisplay(phone),
    hasPhone,
    isPhoneVisible,
