...
const getAnonymousId = () => {
  ...
  if (!anonId) {
    anonId =
      "anon_" + Math.random().toString(36).slice(2, 10) + "_" + Date.now();
...
const getUserId = () => {
  try {
    const state = getAppStore()?.getState?.();
    return (
      state?.UserSignup?.data?.user_id || state?.UserSignup?.data?.id || null
    );
  } catch (e) {
    return null;
  }
...
const getAttributionSource = ({
  ...
  if (searchId) {
    return {
      source: "search",
      source_detail: `search:${searchId}`.slice(0, 100),
    };
  }
  ...
  if (utmSource || utmMedium || utmCampaign) {
    const detail = [`utm:${utmSource || ""}`, utmMedium, utmCampaign]
      .filter(Boolean)
      .join("|");
...
export const useItemTracking = (itemId, options = {}) => {
  ...
  const trackView = useCallback(async (source = null, sourceDetail = null) => {
    ...
    const explicitDetail =
      typeof source === "object"
        ? source?.source_detail || source?.sourceDetail
        : sourceDetail;
  ...
  const trackEngagement = useCallback(
    async (engagementType, extraData = {}, context = {}) => {
      const currentItemId = itemIdRef.current;
      if (!currentItemId) return;
...
export const useEngagementTracking = (itemId) => {
  ...
  return {
    ...
    trackImageView: (imageIndex) =>
      trackEngagement("image_view", { index: imageIndex }),
    ...
    trackSellerOtherItemsClick: () =>
      trackEngagement("seller_other_items_click"),
...
export const useSearchTracking = () => {
  ...
  const trackSearchImpressions = useCallback(async (itemIds, searchData) => {
    ...
    const response = await trackingRequest(
      "track/search-impressions",
      {
        ...
        page,
        results_total:
          searchData?.results_total ?? searchData?.results_count ?? null,
    ...
    const trackedImpressionId =
      response?.payload?.data?.impression_id ||
  ...
  const trackSearchClick = useCallback(
    async (itemId, position, impressionId = null, searchContext = null) => {
      ...
      const response = await trackingRequest("track/search-click", payload, {
        returnPayload: true,
      });
      if (!response?.ok) {
