...
// AŽURIRANA FUNKCIJA: Izvlači Godište, Gorivo, Mjenjač, Pogon i Stanje
const getKeyAttributes = (item) => {
  const attributes = [];
  const customFields = item?.translated_custom_fields || [];

  // Helper funkcija za traženje vrijednosti (da ne ponavljamo kod)
  ...
  // Ako nismo našli ništa od navedenog, koristi fallback metodu
...
// Fallback funkcija (stari smart tags)
    ...
    >
      ...
      >
        ...
        {item?.is_feature && !isViewMoreSlide && (
          <div className="flex items-center gap-1 py-1 px-2 bg-primary rounded-full shadow-md absolute top-2 ltr:left-2 rtl:right-2 z-10">
      ...
      <div className="flex flex-col gap-1.5 p-2 flex-grow">
        ...
        {/* KEY ATTRIBUTES - Prikazuje Godište, Gorivo, Mjenjač, Pogon, Stanje */}
