...
const formatLastSeen = (timestamp) => {
  ...
  if (diffInSeconds < 3600)
    return `Prije ${Math.floor(diffInSeconds / 60)} min`;
  if (diffInSeconds < 86400)
    return `Prije ${Math.floor(diffInSeconds / 3600)} h`;
...
const getItemStatusBadge = (status) => {
  switch (status) {
    case "sold":
      return (
        <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-100 text-red-600 font-bold border border-red-200">
          PRODANO
        </span>
      );
    case "reserved":
      return (
        <span className="text-[10px] px-1.5 py-0.5 rounded bg-orange-100 text-orange-600 font-bold border border-orange-200">
          REZERVIRANO
        </span>
      );
    case "active":
    default:
      return (
        <span className="text-[10px] px-1.5 py-0.5 rounded bg-green-100 text-green-600 font-bold border border-green-200">
          NA PRODAJU
        </span>
      );
  }
  ...
  if (isSearchMode) {
    return (
      <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm z-10 h-[72px]">
        <button
          onClick={closeSearch}
          className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-500 dark:text-slate-300"
        >
          <MdArrowBack size={22} />
        </button>
        ...
        <button
          onClick={closeSearch}
          className="text-sm font-medium text-slate-500 dark:text-slate-300 hover:text-slate-800 dark:hover:text-slate-100 px-2"
        >
          Otkaži
        </button>
      </div>
    ...
    <div className="flex items-center justify-between gap-2 px-4 py-3 border-b border-slate-200/80 dark:border-slate-700 bg-white/95 dark:bg-slate-900/90 backdrop-blur z-10 min-h-[72px]">
      <div className="flex items-center gap-3 min-w-0 flex-1">
        {!isLargeScreen && (
          ...
          >
            <MdArrowBack
              size={22}
              className="rtl:scale-x-[-1] text-slate-600"
            />
          </button>
        )}
      ...
      <div className="flex items-center gap-2 sm:gap-3 pl-2 border-l border-slate-100 dark:border-slate-700 ml-2">
        <div className="hidden md:flex flex-col items-end min-w-0 text-right">
          ...
          <span
            className="text-xs text-slate-500 dark:text-slate-400 max-w-[120px] truncate block"
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent
            ...
            className="w-56 bg-white dark:bg-slate-900 z-50"
          >
            {userData?.phone && (
