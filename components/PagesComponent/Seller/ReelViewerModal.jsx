...
const loadYouTubeIframeApi = () => {
  ...
  ytIframeApiPromise = new Promise((resolve, reject) => {
    ...
    if (!existingScript) {
      ...
      script.onerror = () =>
        reject(new Error("Ne mogu učitati YouTube player API."));
...
const ReelViewerModal = ({
  ...
  const ytPlayerWrapIdRef = useRef(
    `reel-yt-player-${Math.random().toString(36).slice(2, 10)}`,
  );
  ...
  useEffect(() => {
    ...
    if (sellersProp?.length) {
      const nextSellerIndex = Math.min(
        initialSellerIndex,
        sellersProp.length - 1,
      );
      const nextItemIndex = Math.min(
        initialItemIndex,
        (sellersProp[nextSellerIndex]?.items?.length || 1) - 1,
      );
  ...
  const toggleMute = useCallback(
    (e) => {
      ...
      if (isInstagramEmbed) {
        if (!nextMuted) {
          toast.info(
            "Za Instagram zvuk koristite kontrolu zvuka unutar samog videa.",
          );
        }
        return;
      }
        ...
        try {
          if (nextMuted) {
            ...
            const currentVolume = Number(
              ytPlayerRef.current.getVolume?.() || 0,
            );
            if (!Number.isFinite(currentVolume) || currentVolume <= 0) {
              ytPlayerRef.current.setVolume?.(70);
            }
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      if (!was)
        itemStatisticsApi
          .trackFavorite({ item_id: item.id, added: true })
          .catch(() => {});
    } catch {
      item.is_liked = was;
      item.total_likes = num(item.total_likes) + (was ? 1 : -1);
    ...
    <AnimatePresence mode="wait">
      {open && (
        <motion.div
          ...
          className="fixed inset-0 z-[2147483000] bg-black/95 flex items-center justify-center"
        >
          {/* ── story container ── */}
          <motion.div
            key={`seller-${sIdx}`}
              ...
              >
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className="text-white text-[13px] font-semibold truncate max-w-[110px]">
                      {sName}
                    </span>
                    {shop && (
                      <MembershipBadge
                  ...
                  <div className="flex items-center gap-2 text-[11px] text-white/45">
                    ...
                    {isYouTube && (
                      <span className="px-1.5 py-0.5 rounded-full bg-white/15 text-white/70 text-[10px]">
                        YouTube
                      </span>
                    )}
                    {isInstagramEmbed && (
