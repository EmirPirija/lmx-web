...
const EditComponentThree = ({
  ...
  const isTempUpload = (v) =>
    Boolean(
      v &&
      typeof v === "object" &&
      extractTempUploadId(v) &&
      v?.url &&
      !v?.image,
    );
  // Postojeća slika iz DB-a (najčešće { id, image: "https://..." })
  const isExistingDbImage = (v) =>
    !!(
      v &&
      typeof v === "object" &&
      v.id &&
      (v.image || v.original_url || v.path)
    );
  ...
  const handleRemoveVideo = async () => {
    ...
    if (isTempUpload(uploadedVideo) && videoTempId)
      await deleteTemp(videoTempId);
          ...
          >
            ...
            <div
              className={cn(
                ...
                isCompactMode
                  ? "w-10 h-10 bg-primary/10 text-primary rounded-lg"
                  : "p-4 rounded-full bg-white text-slate-400 group-hover:text-primary shadow-sm",
              )}
            >
              {isCompactMode ? (
                ...
                >
                  ...
                  >
                    <button
                      ...
                      className={cn(
                        "rounded-full bg-white/90 p-1.5 text-red-500 shadow-md transition-colors",
                        isMobileImageUi
                          ? "active:bg-red-50"
            ...
            {!uploadedVideo ? (
              <label
                ...
                className={cn(
                  ...
                  isDraggingVideo
                    ? "border-red-400 bg-red-50 scale-[1.01] dark:bg-red-500/10"
