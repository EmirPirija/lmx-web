      ...
      p.map((it) =>
        it.id === itemId
          ? {
              ...
              total_likes: num(it.total_likes) + (it.is_liked ? -1 : 1),
            }
          : it,
      ),
    );
    try {
      const r = await manageFavouriteApi.manageFavouriteApi({
        item_id: itemId,
      });
      if (r?.data?.error !== false) throw new Error();
      itemStatisticsApi
        .trackFavorite({ item_id: itemId, added: next })
        .catch(() => {});
    } catch {
      setItems((p) =>
        p.map((it) =>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <svg
              ...
              viewBox="0 0 16 16"
              id="Ins-Fill--Streamline-Mingcute-Fill"
              height="36"
              width="36"
            >
              <g fill="none" fillRule="nonzero">
                ...
                <path
                  fill="#0ab6af"
                  d="M10.666666666666666 2a3.333333333333333 3.333333333333333 0 0 1 3.333333333333333 3.333333333333333v5.333333333333333a3.333333333333333 3.333333333333333 0 0 1 -3.333333333333333 3.333333333333333H5.333333333333333a3.333333333333333 3.333333333333333 0 0 1 -3.333333333333333 -3.333333333333333V5.333333333333333a3.333333333333333 3.333333333333333 0 0 1 3.333333333333333 -3.333333333333333h5.333333333333333Zm-2.6666666666666665 3.333333333333333a2.6666666666666665 2.6666666666666665 0 1 0 0 5.333333333333333 2.6666666666666665 2.6666666666666665 0 0 0 0 -5.333333333333333Zm0 1.3333333333333333a1.3333333333333333 1.3333333333333333 0 1 1 0 2.6666666666666665 1.3333333333333333 1.3333333333333333 0 0 1 0 -2.6666666666666665Zm3 -2.333333333333333a0.6666666666666666 0.6666666666666666 0 1 0 0 1.3333333333333333 0.6666666666666666 0.6666666666666666 0 0 0 0 -1.3333333333333333Z"
                  strokeWidth="0.6667"
                ></path>
              </g>
            </svg>
                    ...
                    <div className="w-[68px] h-[68px] rounded-full p-[3px] bg-gradient-to-br from-[#F7941D] via-[#E1306C] to-[#833AB4] group-hover:shadow-lg group-hover:shadow-pink-500/20 transition-shadow duration-300">
                      <div className="w-full h-full rounded-full p-[2px]">
                        <UserAvatarMedia
                          ...
                          fallbackClassName="bg-black/25 dark:bg-black/30 backdrop-blur-[1px] text-white/90"
                          iconClassName="w-6 h-6 text-white/90"
                        />
                      </div>
                    </div>
        ...
        <div className="container">
          <div className="flex gap-3 overflow-hidden">
            {[...Array(5)].map((_, i) => (
              <div
                key={i}
                className="shrink-0 w-[180px] sm:w-[220px] aspect-[9/16] rounded-2xl bg-slate-200 dark:bg-slate-800 relative overflow-hidden"
              >
                <div
...
const ReelCard = memo(
  ({
    ...
    const sellerStoryIndex = useMemo(() => {
      ...
      const idx = sellerGroup.items.findIndex(
        (groupItem) => String(groupItem?.id) === String(item?.id),
      );
      return idx >= 0 ? idx : 0;
    }, [sellerGroup, item?.id]);
    ...
    /* speed update */
    useEffect(() => {
      if (!isEmbedSource && shouldLoadVideo && videoRef.current)
    ...
    const handleMessageSeller = async (e) => {
      ...
      if (!sid) {
        toast.error("Prodavač nije pronađen.");
        return;
      }
    ...
    const sendMessage = async () => {
      ...
      try {
        ...
        if (item?.id) {
          try {
            const c = await itemConversationApi.checkConversation({
              item_id: item.id,
            });
            cid =
              c?.data?.data?.conversation_id || c?.data?.data?.item_offer_id;
          } catch {}
          if (!cid) {
            const s = await itemConversationApi.startItemConversation({
              item_id: item.id,
            });
            cid =
              s?.data?.data?.conversation_id ||
              s?.data?.data?.item_offer_id ||
              s?.data?.data?.id;
          }
        }
        if (!cid) {
          try {
            const c = await itemConversationApi.checkDirectConversation({
              user_id: sid,
            });
            cid =
              c?.data?.data?.conversation_id || c?.data?.data?.item_offer_id;
          ...
          if (!cid) {
            const s = await itemConversationApi.startDirectConversation({
              user_id: sid,
            });
            cid =
              s?.data?.data?.conversation_id ||
        ...
        const r = await sendMessageApi.sendMessage({
          item_offer_id: cid,
    ...
    const handleShare = (e) => {
      ...
      if (navigator.share) {
        ...
        navigator.clipboard
          ?.writeText(url)
          .then(() => toast.success("Link kopiran!"))
          .catch(() => {});
      }
    };
          ...
          >
            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#F7941D] via-[#E1306C] to-[#833AB4] p-[1.5px] shrink-0">
              <UserAvatarMedia
                ...
                roundedClassName="rounded-full"
                imageClassName="w-full h-full rounded-full object-cover"
                fallbackClassName="bg-black/25 backdrop-blur-[2px] border border-white/45 text-white/90"
            ...
            <div className="flex items-center gap-1 min-w-0">
              ...
              {isYouTube && (
                <span className="px-1.5 py-0.5 rounded bg-white/20 text-[8px] font-bold text-white uppercase tracking-wide">
                  YT
                </span>
              )}
              {isInstagramEmbed && (
              ...
              {showMenu && (
                ...
                >
                  {[
                    {
                      icon: MdAutorenew,
                      label: autoPlay ? "Auto: uključeno" : "Auto: isključeno",
                      fn: () => onToggleAutoPlay?.(),
                    },
                    {
                      icon: MdPlayArrow,
                      label: autoStory
                        ? "Auto priče: uključeno"
                        : "Auto priče: isključeno",
                      fn: () => onToggleAutoStory?.(),
                    },
                    {
                      icon: MdSpeed,
                      label: `Brzina: ${playSpeed}x`,
                      fn: () => onCycleSpeed?.(),
                    },
                    {
                      icon: MdShuffle,
                      label: shuffled ? "Mix: uključen" : "Mix: isključen",
                      fn: () => onToggleShuffle?.(),
                    },
                    {
                      icon: MdOpenInNew,
                    ...
                    },
                    {
                      icon: MdFlag,
                      label: "Prijavi",
                      fn: () => toast.info("Prijava zabilježena. Hvala!"),
                    },
                  ].map((o, i) => (
                    <button
                      key={i}
              ...
              <div className="flex items-center gap-1 bg-white/90 dark:bg-slate-800/90 backdrop-blur-xl rounded-full px-2.5 py-1.5 shadow-lg border border-white/30">
                <input
                  ...
                  onKeyDown={(e) => {
                    ...
                    if (e.key === "Escape") setShowMsgInput(false);
                  }}
                  placeholder="Poruka..."
