  ...
  return (
    <div className="flex flex-col bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="p-5 lg:p-6 relative">
        <div
          className={cn(
            "prose prose-sm prose-slate lg:prose-base max-w-none leading-relaxed overflow-hidden transition-[max-height] duration-500 ease-in-out dark:prose-invert prose-p:text-inherit prose-strong:text-inherit prose-li:text-inherit prose-headings:text-inherit [&_*]:!text-inherit [&_a]:!text-primary",
      ...
      {isOverflowing && (
        <div className="px-5 lg:px-6 pb-5 pt-0">
          ...
          >
            <span>
              {showFullDescription ? "Prikaži manje" : "Pročitaj detaljan opis"}
            </span>
            {showFullDescription ? (
