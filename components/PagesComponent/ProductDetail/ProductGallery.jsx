...
const ProductGallery = ({
  galleryImages,
  videoData,
  directVideo, // ✅ NEW: direct video file url (npr. product.video)
  productDetails,
  // ✅ TRACKING PROPS
  onGalleryOpen,
  onImageView,
  ...
  // ✅ prefer directVideo, fallback to videoData.url
  const videoUrl = directVideo || videoData?.url || "";
  ...
  const videoIndex = hasVideo ? safeGalleryImages.length : -1;
  const isVideoSelected = selectedIndex === videoIndex;

  // Extract YouTube thumbnail (samo ako je youtube)
  ...
  // ✅ Thumbnail:
  // - ako je youtube -> youtube thumb
  // - inače videoData.thumbnail
  // - fallback prva slika iz galerije
  // - fallback placeholder
  const videoThumbnail =
    getYouTubeThumbnail(videoData?.url) ||
    videoData?.thumbnail ||
    safeGalleryImages?.[0] ||
    placeHolderImage;
  ...
  // ✅ TRACK IMAGE VIEW KADA SE PROMIJENI INDEX (samo za slike)
  useEffect(() => {
    if (
      selectedIndex >= 0 &&
          ...
          <div className="aspect-[870/500] rounded-2xl overflow-hidden bg-black relative">
            ...
            {isVideoPlaying && (
              <>
                ...
                <ReactPlayer
                  url={videoUrl} // ✅ FIX: koristi directVideo ili videoData.url
                  controls
